import psycopg2
from config.db_config import USER, PASSWORD, DBNAME
...
def show_exists_for_user(show: str, user: int) -> bool:
    con = start_connection()
    cur = con.cursor()
    query = f"SELECT * FROM public.\"Series\" NATURAL JOIN public.\"Users\" WHERE discord_id = %s AND name = %s"
...
def increment_current_se(show_id: int):
    prequel_query = f"SELECT series_id, name FROM public.\"Series\" WHERE prequel = %s"
    con = start_connection()
    cur = con.cursor()
